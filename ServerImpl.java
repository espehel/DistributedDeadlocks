...
public class ServerImpl extends UnicastRemoteObject implements Server
{
	...
	public void receiveProbe(ArrayList<Integer> edges) throws RemoteException{
		...
		if(activeTransaction != null){
			//the resoruce this active transaction is waiting for
			ResourceAccess waitingOnRes = activeTransaction.getWaitingForResource();
			//this transaction is waiting for an resoruce
			if(waitingOnRes != null)
				//if this transaction creates an cycle
				if(createsCycle(activeTransaction.getTransactionId(), edges)){
					//the server that holds the resource this transaction is waiting for handles the aborting of this transaction
					waitingOnRes.server.abortTransaction(waitingOnRes.resourceId, activeTransaction.getTransactionId());
				}
				else{
					//no cycle will be made, so we add this transaction to the list of edges
					edges.add(activeTransaction.getTransactionId());
					//the id of the transaction that holds the resource this transaction is waiting on
					int waitingOnTransId = waitingOnRes.server.getResourceHolder(waitingOnRes.resourceId);
					//the server thats owns the transaction this transaction is waiting on
					Server waitingOnServer = getServer(ServerImpl.getTransactionOwner(waitingOnTransId));
					if(waitingOnServer != null)
						//if that server exist we send the probe to it
						waitingOnServer.receiveProbe(edges);
				}
		}
	}

	//uses the id to find the right resoruce and tells it to wake all transactions waiting
	public void abortTransaction(int resourceId, int transactionId) throws RemoteException{
		Resource r = resources.get(resourceId);
		synchronized (r) {
			//tells resrouce wich transactin that needs to abort
			r.abortTransactionId = transactionId;
			//wakes all the resources transactions
			r.notifyAll();
		}
	}
	//checks if the transaction will create an edge
	...
	/**
	 * Returns the TransactionId of the owner of the lock to this resource
	 */
	@Override
