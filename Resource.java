...
class Resource
{
	...
	synchronized boolean lock(int transactionId)
	{
		...
		//loggs the time before thread going to sleep for timeout purposes
		long start = System.currentTimeMillis();
		while (lockOwner != NOT_LOCKED) {
			try {
				//the thread will wake after 3000 if not woken before. When it is woken it will check if the time has gone past the timeout, if so it aborts
				if(Globals.USE_TIMEOUT){
					wait(3000);
					if(start<System.currentTimeMillis()-Globals.TIMEOUT_INTERVAL){
						return false;
					}
				}
				//if a deadlock is detected all threads waiting on this object will be woken. All will go back waiting except one who will be aborted
				if(Globals.PROBING_ENABLED){
					wait();
					//finds the transaction that needs to be aborted
